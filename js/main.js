...
profileList.addEventListener("click", event => {
  ...
  if (!removeBtn) return;
  faceProfiles = deleteFaceProfile(faceProfiles, removeBtn.dataset.deleteProfile);
  learnStatus.textContent = faceProfiles.length ? "PROFILE DELETED" : "NO MEMORY";
  updateMemoryUi();
});

function renderProfiles(force = false) {
  const renderKey = JSON.stringify(faceProfiles.map(p => [p.id, p.nickname, p.image]));
...
function drawEmojiMasks(renderCtx, maskList) {
  maskList.forEach(({ box, emotion, masked }) => {
    if (!masked) return;
    const [x1, y1, x2, y2] = box;
    const tw = x2 - x1, th = y2 - y1;
    const img = imgs[emotion] || imgs.neutral;
    if (tw >= MIN_BOX && th >= MIN_BOX && img) renderCtx.drawImage(img, x1, y1, tw, th);
  });
}

function drawBlurMasks(renderCtx, maskList, W, H) {
  maskList.forEach(({ box, masked }) => {
    if (!masked) return;
...
function keepIndicesForMode(boxes, rememberedMatches) {
  if (targetMode === "remembered") return new Set(rememberedMatches.matchedIndices);
...
function collectLearningSample(embeddings, boxes, now, sourceCtx) {
  ...
  if (learnSamples.length >= LEARN_SAMPLES) {
    ...
    const result = addFaceProfile(faceProfiles, template, learnSamples.length, learnImage);
    ...
    if (result.duplicate) {
      ...
      learnStatus.textContent = result.profile ? `${result.profile.nickname} REMEMBERED` : "LEARN FAILED";
    }
    updateMemoryUi();
  }
}
...
function loop() {
  ...
  const embeddings = faces.map((lms, i) => faceEmbedding(lms, sampleCtx, boxes[i]));
  const rememberedMatches = findRememberedFaces(embeddings, faceProfiles, MATCH_THRESHOLD);
  ...
  else if (targetMode === "remembered") footStatus.textContent = "NO FACE MEMORY";
